...
class lectioToCalendar:
        ...
        if not self.creds or not self.creds.valid:
            if (
                ...
                flow = InstalledAppFlow.from_client_secrets_file(
                    str(self.server_dir) + "/credentials.json", self.SCOPES
                )
    ...
    def getFormattedSchedule(self, week):
        ...
        for lectio_event in lectio_scheme:
            if (
                lectio_event["Status"] == "Aflyst!"
                or "studiecafe" in str(lectio_event["Title"]).lower()
            ):
                continue
            ...
            if lectio_event["Title"] == " ":
                ...
                if lectio_event["Team"] != " ":
                    summary = (
                        lectio_event["Title"]
                        + " - "
                        + self.lessonCodeToText(lectio_event["Team"])
                    )
                else:
            ...
            if lectio_event["Note"] != []:
                ...
                if note_txt != "<ul>":
                    ...
                    description += (
                        "<b>Lektier:</b> " + note_txt.replace("[...]", "") + "</ul>"
                    )
                else:
            ...
            if lectio_event["Id"] != " ":
                description += (
                    '<a href="https://www.lectio.dk/lectio/'
                    + self.school_id
                    + "/"
                    + lectio_event["EventLink"]
                    + lectio_event["Id"]
                    + '">Læs mere</a>'
                )
            ...
            formatted_date = (
                date[1] + "-" + date[0].split("/")[1] + "-" + date[0].split("/")[0]
            )

            dateTime_start = (
                formatted_date + "T" + times[0].split()[1] + ":00+02:00"
            )  # normally +01:00, but had to adjust for daylight savings
            ...
            event = {
                ...
                "source.url": "https://www.lectio.dk/lectio/"
                + self.school_id
            ...
            if lectio_event["Id"] == " ":
                ...
                event["id"] = dateTime_start.replace("T", "").replace("+", "").replace(
                    ":", ""
                ).replace("-", "") + dateTime_end.replace("T", "").replace(
                    "+", ""
                ).replace(
                    ":", ""
                ).replace(
    ...
    def updateCalendar(self, weekSchedule):
        ...
        first_event_datetime = datetime.datetime.strptime(
            weekSchedule[0]["start"]["dateTime"][:-6], "%Y-%m-%dT%H:%M:%S"
        )
        week_start_datetime = (
            first_event_datetime
            - datetime.timedelta(days=first_event_datetime.weekday())
        ).replace(minute=0, hour=0, second=0)
        week_end_datetime = (week_start_datetime + datetime.timedelta(days=6)).replace(
        ...
        events = (
            ...
            .list(
                ...
                timeMin=week_start_datetime.strftime("%Y-%m-%dT%H:%M:%S+02:00"),
                timeMax=week_end_datetime.strftime("%Y-%m-%dT%H:%M:%S+02:00"),
            )
            .execute()
        )
        ...
        for extra_event in extra_events:
            ...
            try:
                self.service.events().delete(
                    calendarId=self.calendarId, eventId=extra_event
                ).execute()
            except:
                ...
                if event["id"] == missing_event:
                    ...
                    try:
                        self.service.events().insert(
                            calendarId=self.calendarId, body=event
                        ).execute()
                    except HttpError:
        ...
        for i in weekSchedule:
            if i["id"] not in extra_events and i["id"] not in missing_events:
                try:
                    ...
                    self.service.events().update(
                        calendarId=self.calendarId, eventId=i["id"], body=i
                    ).execute()
                except:
