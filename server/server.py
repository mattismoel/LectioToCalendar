...
def updateCurrentWeek():
    ...
    while True:
        ...
        if datetime.today().weekday() >= 5:
            week_to_update = str(today_date[1] + 1)
        else:
            week_to_update = str(today_date[1])
        ...
        try:
            ...
            raise Exception(
                "Failed to establish connection to Lectio. Thread 1 stopped."
            )
        ...
        print(
            (
                ...
                + " - Waiting "
                + str(current_week_sync_interval * 60)
                + "s..."
            ),
            flush=True,
        )
...
def updateFutureWeeks():
    ...
    while True:
        ...
        try:
            ...
            raise Exception(
                "Failed to establish connection to Lectio. Thread 2 stopped."
            )
        ...
        print(
            (
                ...
                + datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                + " - Waiting "
                + str(future_week_sync_interval * 60)
                + "s..."
            ),
            flush=True,
        )
